...
public sealed class BNRProvider : CentralBankProviderBase {
	...
	public BNRProvider(HttpClient http, IConfiguration configuration, CentralBankEntity bank, ILogger<BNRProvider> logger) : base(http, bank, configuration) {
		_logger = logger;
	}
		...
		for (var date = fromDate; date <= toDate; date = date.AddDays(1)) {
			try {
				...
				foreach (var rateElement in document.Descendants().Where(e => e.Name.LocalName == "Rate")) {
					...
					results.Add(new ExchangeRateResult(date, Bank.NativeCurrency.CurrencyCode, quoteCurrency, rate / multiplier, Bank.BankCode));
				}
			}
			catch (Exception ex) {
